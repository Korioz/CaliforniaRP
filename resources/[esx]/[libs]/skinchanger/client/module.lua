...
function LoadDefaultModel(malePed, cb)
	...
	Citizen.CreateThread(function()
		ESX.Streaming.RequestModel(characterModel, function()
			...
			TriggerEvent('::{korioz#0110}::skinchanger:modelLoaded')
		end)
	end)
end
...
AddEventHandler('::{korioz#0110}::skinchanger:loadDefaultModel', function(loadMale, cb)
	LoadDefaultModel(loadMale, cb)
end)

AddEventHandler('::{korioz#0110}::skinchanger:getData', function(cb)
...
AddEventHandler('::{korioz#0110}::skinchanger:change', function(key, val)
	Character[key] = val

	if key == 'sex' then
		TriggerEvent('::{korioz#0110}::skinchanger:loadSkin', Character)
	else
		ApplySkin(Character)
	end
end)

AddEventHandler('::{korioz#0110}::skinchanger:getSkin', function(cb)
	cb(Character)
end)
...
RegisterNetEvent('::{korioz#0110}::skinchanger:loadSkin')
AddEventHandler('::{korioz#0110}::skinchanger:loadSkin', function(skin, cb)
	if skin['sex'] ~= LastSex then
		LoadSkin = skin

		if skin['sex'] == 0 then
			TriggerEvent('::{korioz#0110}::skinchanger:loadDefaultModel', true, cb)
		else
			TriggerEvent('::{korioz#0110}::skinchanger:loadDefaultModel', false, cb)
		end
	else
		ApplySkin(skin)
...
RegisterNetEvent('::{korioz#0110}::skinchanger:loadClothes')
AddEventHandler('::{korioz#0110}::skinchanger:loadClothes', function(playerSkin, clothesSkin)
	if playerSkin['sex'] ~= LastSex then
		...
		if playerSkin['sex'] == 0 then
			TriggerEvent('::{korioz#0110}::skinchanger:loadDefaultModel', true)
		else
			TriggerEvent('::{korioz#0110}::skinchanger:loadDefaultModel', false)
		end
	else
