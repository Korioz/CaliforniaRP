...
function OpenRealestateAgentMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'realestateagent', {
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('::{korioz#0110}::esx_society:openBossMenu', 'realestateagent', function(data, menu) end)
		end
...
function OpenPropertyMenu()
	TriggerEvent('::{korioz#0110}::esx_property:getProperties', function(properties)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'properties', elements, function(data, menu)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell_property_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							_TriggerServerEvent('::{korioz#0110}::esx_realestateagentjob:sell', GetPlayerServerId(closestPlayer), data.data.name, amount)
							menu2.close()
						end
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'rent_property_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							_TriggerServerEvent('::{korioz#0110}::esx_realestateagentjob:rent', GetPlayerServerId(closestPlayer), data.data.name, amount)
							menu2.close()
						end
			...
			elseif data.value == 'gps' then
				TriggerEvent('::{korioz#0110}::esx_property:getProperty', data.data.name, function(property)
					if property.isSingle then
						SetNewWaypoint(property.entering.x, property.entering.y)
					else
						TriggerEvent('::{korioz#0110}::esx_property:getGateway', property, function(gateway)
...
function OpenCustomersMenu()
	ESX.TriggerServerCallback('::{korioz#0110}::esx_realestateagentjob:getCustomers', function(customers)
		local elements = {
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'customers', elements, function(data, menu)
			if data.value == 'revoke' then
				_TriggerServerEvent('::{korioz#0110}::esx_realestateagentjob:revoke', data.data.propertyName, data.data.propertyOwner)
				OpenCustomersMenu()
			elseif data.value == 'gps' then
				TriggerEvent('::{korioz#0110}::esx_property:getProperty', data.data.propertyName, function(property)
					if property.isSingle then
						SetNewWaypoint(property.entering.x, property.entering.y)
					else
						TriggerEvent('::{korioz#0110}::esx_property:getGateway', property, function(gateway)
...
AddEventHandler('::{korioz#0110}::esx_realestateagentjob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{korioz#0110}::esx_realestateagentjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('::{korioz#0110}::esx_realestateagentjob:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('::{korioz#0110}::esx_realestateagentjob:hasExitedMarker', LastZone)
		end
	end
end)
