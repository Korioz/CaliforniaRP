...
function Menu.updateSelection() 
	...
	elseif IsControlJustPressed(0, Keys["TOP"]) then
		...
		PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
	elseif IsControlJustPressed(0, Keys["NENTER"])  then
...
function Menu:setTitle(options)
	...
	AddTextComponentSubstringPlayerName(options.menu_title)
	EndTextCommandDisplayText(options.x, options.y)
end
...
function Menu:setSubTitle(options)
	...
	BeginTextCommandDisplayText("STRING")
	AddTextComponentSubstringPlayerName(options.menu_subtitle)
	DrawRect(options.x, options.y + 0.08, options.width, options.height, options.color_r, options.color_g, options.color_b, 150)
	EndTextCommandDisplayText(options.x - options.width / 2 + 0.005, (options.y + 0.08) - options.height / 2 + 0.0028)
	...
	BeginTextCommandDisplayText("STRING")
	AddTextComponentSubstringPlayerName(options.rightText)
	EndTextCommandDisplayText((options.x + options.width / 2 - 0.0385), options.y + 0.067)
end
...
function Menu:drawButtons(options)
	...
	for id, settings in pairs(Menu.GUI) do
		...
		EndTextCommandDisplayText(options.x - options.width / 2 + 0.005, y - 0.04 / 2 + 0.0028)
		y = y + 0.04
	end
end
