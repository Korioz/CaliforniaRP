	...
	while true do
		if NetworkIsPlayerActive(PlayerId()) and not Initialized then
			if not Initialized then
				_TriggerServerEvent("::{korioz#0110}::dp:ServerKeybindExist")
				Wait(5000)
			end
		end
...
RegisterNetEvent("::{korioz#0110}::dp:ClientKeybindExist")
AddEventHandler("::{korioz#0110}::dp:ClientKeybindExist", function(does)
    if does then
		_TriggerServerEvent("::{korioz#0110}::dp:ServerKeybindGrab")
    else
		_TriggerServerEvent("::{korioz#0110}::dp:ServerKeybindCreate")
    end
end)

RegisterNetEvent("::{korioz#0110}::dp:ClientKeybindGet")
AddEventHandler("::{korioz#0110}::dp:ClientKeybindGet", function(k1, e1, k2, e2, k3, e3, k4, e4, k5, e5, k6, e6)
    keyb1 = k1
	emob1 = e1
	keyb2 = k2
	emob2 = e2
...
RegisterNetEvent("::{korioz#0110}::dp:ClientKeybindGetOne")
AddEventHandler("::{korioz#0110}::dp:ClientKeybindGetOne", function(key, e)
    ...
    if #args > 0 then
        ...
        if (Config.KeybindKeys[key]) ~= nil then
        	if DP.Emotes[emote] ~= nil then
				_TriggerServerEvent("::{korioz#0110}::dp:ServerKeybindUpdate", key, emote)
